...
import { Crc32Provider } from '../crc32/crc32';

/*
  Generated class for the MnemonicProvider provider.
...
export class MnemonicProvider {
  mn_default_wordset = 'english';
  constructor(
    public http: HttpClient,
   public crc32:Crc32Provider
...
mn_get_checksum_index(words, prefix_len) {
    var trimmed_words = "";
    for (var i = 0; i < words.length; i++) {
        trimmed_words += words[i].slice(0, prefix_len);
    }

    var checksum = this.crc32.run(trimmed_words);
    var index = checksum % words.length;
    return index;
}

mn_encode(str, wordset_name) {
   
    wordset_name = wordset_name || this.mn_default_wordset;
    var wordset = this.mn_words[wordset_name];
    var out = [];
...
mn_decode(str, wordset_name) {
    ...
    for (var i = 0; i < wlist.length; i += 3) {
        ...
        out += this.mn_swap_endian_4byte(('0000000' + x.toString(16)).slice(-8));
    }
    // Verify checksum
    if (wordset.prefix_len > 0) {
        var index = this.mn_get_checksum_index(wlist, wordset.prefix_len);
...
mn_words = {
  ...
  initWords(){
    for (var i in this.mn_words) {
      if (this.mn_words.hasOwnProperty(i)) {
          if (this.mn_words[i].prefix_len === 0) {
              continue;
          }
          this.mn_words[i].trunc_words = [];
          for (var j = 0; j < this.mn_words[i].words.length; ++j) {
              this.mn_words[i].trunc_words.push(this.mn_words[i].words[j].slice(0, this.mn_words[i].prefix_len));
          }
      }
    }
  }
